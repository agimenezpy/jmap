...
def calle_simple(request):
    ...
    if params.has_key("query"):
        ...
        result["items"] = Via.objects.extra(where=["to_tsvector('spanish', translate(upper(nombre), 'ÁÉÍÓÚÜ', 'AEIOUU')) @@ to_tsquery(translate(upper(%s), 'ÁÉÍÓÚÜ', 'AEIOUU'))"], params=[nombre])
...
def lugar(request):
    ...
    if params.has_key("nombre") and params.has_key("tipo") and len(params["nombre"]) > 0 and len(params["tipo"]) > 0:
        ...
        result["items"] = qs.extra(where=[u"to_tsvector('spanish', translate(upper(nombre), 'ÁÉÍÓÚÜ', 'AEIOUU')) @@ to_tsquery(translate(upper(%s), 'ÁÉÍÓÚÜ', 'AEIOUU'))"], params=[nombre])
        result["count"] = len(result["items"])
    ...
    if params.has_key("nombre") and params.has_key("entre") and params.has_key("barrio") and params.has_key("zona"):
        ...
        if len(params["entre"]) > 0:
            ...
            cursor.execute(u"""SELECT c1.id, c2.id, c1.abrev, c2.abrev, asewkb(intersection(c1.the_geom, c2.the_geom))
                           ...
                           WHERE to_tsvector('spanish', translate(upper(c1.nombre), 'ÁÉÍÓÚÜ', 'AEIOUU')) @@ to_tsquery(translate(upper(%s), 'ÁÉÍÓÚÜ', 'AEIOUU'))
                             AND to_tsvector('spanish', translate(upper(c2.nombre), 'ÁÉÍÓÚÜ', 'AEIOUU')) @@ to_tsquery(translate(upper(%s), 'ÁÉÍÓÚÜ', 'AEIOUU'))
